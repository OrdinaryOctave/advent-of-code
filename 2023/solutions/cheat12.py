...
with open("2023/inputs/day12") as f:
    ws = [l.split() for l in f.read().strip().split("\n")]

functionResults = {}

def num_solutions(s, sizes, num_done_in_group=0):
    if (s, sizes, num_done_in_group) in functionResults:
        return functionResults[(s, sizes, num_done_in_group)]
    if not s:
        # Is this a solution? Did we handle and close all groups?
        functionResults[(s, sizes, num_done_in_group)] = not sizes and not num_done_in_group
    ...
    functionResults[(s, sizes, num_done_in_group)] = num_sols
    return num_sols
